...
export default function StudentLayout({ children, title }) {
  ...
  const sidebarItems = [
    { icon: 'dashboard', label: 'Dashboard', path: '/dashboard', active: location.pathname === '/dashboard' || location.pathname === '/student' },
    { icon: 'menu_book', label: 'My Courses', path: '/my-courses', active: location.pathname.startsWith('/my-courses') || location.pathname.includes('/student/course/') },
    { icon: 'assignment', label: 'Assignments', path: '/assignments', active: location.pathname === '/assignments' || location.pathname.startsWith('/assignment/') },
    { icon: 'school', label: 'Grades', path: '/grades', active: location.pathname === '/grades' },
    { icon: 'calendar_today', label: 'Calendar', path: '/calendar', active: location.pathname === '/calendar' },
    { icon: 'chat_bubble', label: 'Messages', path: '/messages', active: location.pathname === '/messages', count: unreadMessages },
    { icon: 'notifications', label: 'Notifications', path: '/notifications', active: location.pathname === '/notifications', count: unreadNotifications },
  ...
  const mobileNavItems = [
    { icon: 'home', label: 'Home', path: '/dashboard', active: location.pathname === '/dashboard' || location.pathname === '/student' },
    { icon: 'menu_book', label: 'Courses', path: '/my-courses', active: location.pathname.startsWith('/my-courses') || location.pathname.includes('/student/course/') },
    { icon: 'assignment', label: 'Assignments', path: '/assignments', active: location.pathname === '/assignments' || location.pathname.startsWith('/assignment/') },
    { icon: 'chat_bubble', label: 'Messages', path: '/messages', active: location.pathname === '/messages', count: unreadMessages },
    { icon: 'person', label: 'Profile', path: '/student/profile', active: location.pathname === '/student/profile' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex gap-8">
        ...
        <aside className="hidden lg:block w-64 shrink-0 space-y-1">
          <nav className="flex flex-col gap-1.5 bg-white dark:bg-slate-900/60 backdrop-blur-sm p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm">
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest px-3 mb-2">Navigation</p>
