...
const LoginPage = () => {
  ...
  // Mock login users for development
  const mockUsers = [
    { email: 'turing@educore.com', name: 'Dr. Alan Turing', role: 'head_of_department' },
    { email: 'ghopper@educore.com', name: 'Grace Hopper', role: 'instructor' },
    { email: 'admin@educore.com', name: 'Super Admin', role: 'admin' },
    { email: 'jdoe@student.com', name: 'John Doe', role: 'student' },
  ];

  const handleMockLogin = async (mockEmail) => {
    try {
      const user = await mockLogin(mockEmail);
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-xl shadow-xl overflow-hidden border border-slate-200 dark:border-slate-800">
        ...
        <div className="p-8">
          ...
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-700">
            <p className="text-slate-600 dark:text-slate-400 text-xs font-semibold mb-3 uppercase tracking-wider">
              Quick Test Login
            </p>
            <div className="grid grid-cols-2 gap-2">
              {mockUsers.map((user) => (
                <button
                  ...
                  className="text-xs bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 py-2 px-3 rounded-lg transition-colors font-medium"
                >
                  <span className="block truncate">{user.name}</span>
                  <span className="text-xs opacity-75 truncate">({user.role})</span>
                </button>
              ))}
            </div>
          </div>
