...
const DepartmentHeadDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState('overview'); // 'overview', 'course', 'teaching', 'instructor', 'profile'
  ...
  // HOD navigation sidebar links
  const sidebarLinks = [
    { label: 'Overview', to: '#', icon: 'dashboard' },
    { label: 'Course Assignments', to: '#', icon: 'app_registration' },
    { label: 'Teaching Tasks', to: '#', icon: 'assignment_turned_in' },
    { label: 'Instructor Workloads', to: '#', icon: 'group_work' },
    { label: 'Profile', to: '#', icon: 'person' },
    { label: 'Inbox Messages', to: '/messages', icon: 'chat' },
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0f172a] text-slate-800 dark:text-slate-200">
      <Topbar title="Department Head Portal" />

      <div className="flex flex-col md:flex-row">
        {/* HOD-specific sidebar with custom links mapper */}
        <aside className="w-full md:w-64 md:min-h-[calc(100vh-72px)] border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <nav className="p-4 space-y-2">
            {sidebarLinks.map((link) => {
              const linkTabId = link.label.split(' ')[0].toLowerCase();
              const isTab = link.to === '#';
              const isActive = isTab ? activeTab === linkTabId : false;

              return (
                <button
                  key={link.label}
                  onClick={() => isTab && setActiveTab(linkTabId)}
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800">
            <button
              onClick={logout}
              className="w-full flex items-center gap-3 px-3.5 py-3 rounded-xl text-sm font-semibold text-rose-600 hover:bg-rose-500/10 transition-all duration-200 group"
            >
              <span className="material-symbols-outlined text-[20px] transition-transform duration-200 group-hover:scale-110">
                logout
              </span>
